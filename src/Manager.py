...
class Manager:
    def __init__(
        self, n: int, m: int, task_list: list[Task], machine_list: list[Machine]
    ) -> None:
        self._n = n
        self._m = m
        ...
        self._free_task_list = dict()
        for task in task_list:
            self._free_task_list[task.id] = task
    ...
    @property
    def free_task_list(self):
        return self._free_task_list
    ...
    def _add_task_to_machine(self, task_id: int, machine_id: int) -> None:
        ...
        self.free_task_list.pop(task_id)
    ...
    def greedy(self):
        ...
        for task in self.rcl_list:
            print(
                f"task_{task.id}: cores {task.cores}; duration {task.duration}; eraliest start {task.eraliest_start}; latest start {task.latest_start}"
            )
    ...
    def print_solution(self):
        ...
        for machine in self.machine_list.values():
            print(
                f"machine_{machine.id}: cores {machine.cores}; start time {machine.start_time}; shutdown time {machine.end_time}"
            )
            for task_start_time, task_dict in machine.task_queue.items():
                if task_dict is not None:
                    for task_id, task in task_dict.items():
                        ...
                        print(
                            f"\t task_{task_id}: cores {task.cores}; start time {task_start_time}; end time {task_start_time + task.duration}"
                        )
            print()
